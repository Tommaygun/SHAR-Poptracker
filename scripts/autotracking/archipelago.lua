...
ScriptHost:LoadScript("scripts/autotracking/card_mapping.lua")
...
function onClear(slot_data)
    ...
    for _, id in pairs(Archipelago.CheckedLocations) do
        if CARD_MAPPING[id] then
            for _, location in pairs (CARD_MAPPING[id]) do
                if location then
                    CARD_CHECK[location] = true
                end
            end
        end
    end
    ...
    for _, location_order in pairs(CARD_MAPPING) do
        for _, location in pairs(location_order) do
            if location then
                local location_obj = Tracker:FindObjectForCode(location)
                if location_obj then
                    if location:sub(1, 1) == "@" then
...
function onLocation(location_id, location_name)
    if CARD_MAPPING[location_id] ~= nil then
        onCardLocation(location_id)
        return
    end
...
--called when selected card location gets cleared
function onCardLocation(locationID)
    local card = CARD_MAPPING[locationID]
    local card_obj = Tracker:FindObjectForCode(card[1])
    if card_obj then
