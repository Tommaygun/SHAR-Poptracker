...
the_simpsons_hit_and_run_location = {}
the_simpsons_hit_and_run_location.__index = the_simpsons_hit_and_run_location
...
function the_simpsons_hit_and_run_location.new(name)
    local self = setmetatable({}, the_simpsons_hit_and_run_location)
    if name then
...
function the_simpsons_hit_and_run_location:connect_one_way(exit, rule)
    if type(exit) == "string" then
        ...
        if existing then
            ...
            exit = the_simpsons_hit_and_run_location.new(exit)
        end
    end
    if rule == nil then
        rule = always
...
function the_simpsons_hit_and_run_location:connect_two_ways(exit, rule)
...
function the_simpsons_hit_and_run_location:connect_one_way_entrance(name, exit, rule)
    if rule == nil then
        rule = always
    end
    self.exits[#self.exits + 1] = { exit, rule }
...
function the_simpsons_hit_and_run_location:connect_two_ways_entrance(name, exit, rule)
...
-- acts as a shortcut for 2 connect_one_way_entrance-calls
function the_simpsons_hit_and_run_location:connect_two_ways_entrance_door_stuck(name, exit, rule1, rule2)
...
function the_simpsons_hit_and_run_location:connect_two_ways_stuck(exit, rule1, rule2)
...
function the_simpsons_hit_and_run_location:accessibility()
...
-- 
function the_simpsons_hit_and_run_location:discover(accessibility, keys)
...
entry_point = the_simpsons_hit_and_run_location.new("entry_point")
